...
namespace Systems
{
	[Serializable][Documentation(Doc.Traps, Doc.Movement, "this system moving traps")]
    public sealed class MoveTrapSystem : BaseSystem, IUpdatable, IGlobalStart
    {
        [Required]
        ...
        {
            if (trapParameters.IsMoving)
            {
                var currentPosition = Owner.GetPosition();
                var offset = 0f;

                if (trapParameters.IsHorizontal)
                {
